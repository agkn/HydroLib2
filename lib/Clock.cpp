...
DateTime::DateTime(time_t aTicks) {
    mSecond = static_cast<uint8_t>(aTicks % 60);
    aTicks /= 60; // now it is minutes
    mMinute = static_cast<uint8_t>(aTicks % 60);
    aTicks /= 60; // now it is hours
    mHour = static_cast<uint8_t>(aTicks % 24);
    ...
    mYear = static_cast<uint8_t>(tmp - 30); // years since 2000
    for(uint8_t month = 0; month < 12; month++) {
        int m = (month == 1) && IS_LEAP_YEAR(2000 + mYear) ? 29 : monthDays[month];
        if (aTicks < m) {
            mMonth = static_cast<uint8_t>(month + 1);
            mDay = static_cast<uint8_t>(aTicks + 1);
            break;
